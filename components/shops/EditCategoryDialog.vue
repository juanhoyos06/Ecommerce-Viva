    ...
    <v-dialog v-model="dialog" width="60%">
        <v-card>
            <v-toolbar color="#FFCC00" :elevation="8">
                <v-card-title class="card-title">EDITAR CATEGORIA</v-card-title>

            </v-toolbar>

            <v-card-text>
                <form action="javascript:void(0)" @submit="editCategory()">
                    <v-row>
                        <v-col cols="6">
...
export default {
    ...
    methods: {
        ...
        async editCategory() {
            ...
            try {
                this.openEditDialog()
                const url = `${config.api_host}/categories/${this.$props.selectedCategory}`;
                this.category = {
                    'name': this.name,
                ...
                await axios.put(url, this.category, { headers })
                Swal.fire(
                    {
                        icon: 'success',
        ...
        async openEditDialog() {
            const categoryId = this.$props.selectedCategory;
            const url = `${config.api_host}/categories/${categoryId}`;
            const headers = this.getHeaders();
